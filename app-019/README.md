...
| 配合余量表 | 硬木/软木 × 紧/标准/松 六格经验值，可编辑并持久化；缺格逐格补出厂默认，存储损坏自动回退默认表，绝不白屏 |
...
│   │   ├── fit.ts             # 配合余量表（默认值 + 逐格清洗 + localStorage 持久化）
│   │   └── format.ts          # 0.1 / 0.5 步进格式化
│   ├── geometry/views.ts      # 三视图几何（front / top / side 统一数据模型）
│   ├── components/            # ViewSvg（SVG 渲染 + 校验尺）、ParamForm、ErrorBoundary
...
| `wjb.fittable.v1` | 可编辑配合余量表（读取时逐格清洗：缺格/非数字/坏 JSON 自动按出厂默认补齐，六格永远有效） |
...
| 层级 | 数量 | 覆盖点 |
| --- | --- | --- |
| 单元（vitest） | 56 | 随机 200 组燕尾闭合 ≤ 0.1mm（mulberry32 固定种子）、直榫 10 组手工核算、三视图一致性、边界警告、锯切线数量、导出导入往返全等、余量表缺格/坏数据清洗、性能 |
| 组件（RTL） | 15 | 新建流程、键盘 ±0.5 微调、脏参数提示条、类型切换联动、列表筛选、非法导入错误提示、表单钳制、坏余量表下编辑器不白屏、错误边界兜底 |
